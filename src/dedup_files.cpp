...
//Kokkos::parallel_reduce("Count repeat leaves", Kokkos::RangePolicy<>(0, g_nodes.capacity()), KOKKOS_LAMBDA(const uint32_t i, uint32_t& num) {
//  if(g_nodes.valid_at(i)) {
//    uint32_t node = g_nodes.key_at(i);
//    Node info = g_nodes.value_at(i);
//    if(info.nodetype == Repeat && node >= num_chunks-1) {
//      num += 1;
//    }
//  }
//}, num_curr_repeats);
