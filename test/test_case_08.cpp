...
int main(int argc, char** argv) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    step1_h(6) = static_cast<uint8_t>('A');
    ...
    printf("Expected 1 first occurrence region and 1 shifted duplicate regions\n");
    if(deduplicator.first_ocur_updates_d.size() != 1 || deduplicator.shift_dupl_updates_d.size() != 1) {
      res = -1;
    }
  }
  Kokkos::finalize();
