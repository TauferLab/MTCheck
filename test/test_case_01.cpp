...
int main(int argc, char** argv) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    step1_h(2) = static_cast<uint8_t>('K');
    step1_h(3) = static_cast<uint8_t>('L');
    step1_h(4) = static_cast<uint8_t>('M');
    step1_h(5) = static_cast<uint8_t>('N');
    step1_h(6) = static_cast<uint8_t>('O');
    step1_h(7) = static_cast<uint8_t>('P');
    ...
    deduplicator.checkpoint((uint8_t*)(step1_d.data()), step1_d.size(), diff2_h, false);
    ...
    if(deduplicator.num_first_ocur() != 1 || deduplicator.num_shift_dupl() != 0) {
      res = -1;
    }
  }
  Kokkos::finalize();
