...
for(auto set_iter=tree_roots.begin(); set_iter!=tree_roots.end(); set_iter++) {
  ...
  if(labels[node] == FIRST_DUPL) {
    ...
    if((num_match > 0) && (num_match != (leaf_end-leaf_beg+1))) {
      labels[node] = DONE;
    ...
    if(iter != first_ocur_dupl.end()) {
      ...
      auto& node_meta = first_occur_h.value_at(first_occur_h.find(curr_tree(root)));
      node_meta.node = root;
      labels[root] = FIRST_OCUR;
      uint32_t beg = leftmost_leaf(root, num_nodes);
      uint32_t end = rightmost_leaf(root, num_nodes);
