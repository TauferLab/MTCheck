...
//  Kokkos::parallel_for("Restart first occurrence", Kokkos::RangePolicy<>(0, num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
//    uint32_t node;
//    memcpy(&node, metadata_subview.data()+i*sizeof(uint32_t), sizeof(uint32_t));
//    uint32_t datasize = chunk_size;
//    if(node == num_chunks-1)
...
////if(node < 100)
////    printf("Reading entry %u data %lu to buffer offset %lu (i:%u, offset:%lu, num_changes:%u, chunk:%u)\n", i, static_cast<uint64_t>(chunk_size)*static_cast<uint64_t>(node), data_offset-sizeof(header_t)+static_cast<uint64_t>(i)*static_cast<uint64_t>(chunk_size), i, data_offset-sizeof(header_t), num_first_ocur, node);
//    memcpy(data.data()+static_cast<uint64_t>(node)*static_cast<uint64_t>(chunk_size), data_subview.data()+static_cast<uint64_t>(i)*static_cast<uint64_t>(chunk_size), datasize);
//    node_list(node) = NodeID(node, cur_id);
//  });
  Kokkos::fence();
...
//if(cur_id == ref_id)
//printf("Error some chunks not filled %u\n", i);
    }
  });
  Kokkos::fence();
    ...
    Kokkos::parallel_for("Restart Hashlist first occurrence", chunk_policy, KOKKOS_LAMBDA(const TeamMember& team_member) {
      ...
      if(node_list(i).tree == cur_id) {
        ...
        if(distinct_map.exists(id)) {
          ...
          uint32_t* data_u32 = (uint32_t*)(data.data()+static_cast<uint64_t>(chunk_size)*static_cast<uint64_t>(i));
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, writesize/4), [&] (const uint64_t& j) {
            data_u32[j] = buffer_u32[j];
          });
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, writesize%4), [&] (const uint64_t& j) {
            data(static_cast<uint64_t>(chunk_size)*static_cast<uint64_t>(i) + ((writesize/4)*4) + j) = data_subview(offset+((writesize/4)*4)+j);
          });
...
//    distinct_map.clear();
//    distinct_map.rehash(chkpt_header.num_first_ocur);
//    Kokkos::parallel_for("Restart first occurrences", Kokkos::RangePolicy<>(0, chkpt_header.num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
//      uint32_t node;
//      memcpy(&node, metadata_subview.data()+i*sizeof(uint32_t), sizeof(uint32_t));
//      distinct_map.insert(NodeID(node,cur_id), i*chunk_size);
//      if(node_list(node).tree == cur_id) {
//        uint32_t writesize = chunk_size;
//        if(node == num_chunks-1) 
//          writesize = datalen-node*chunk_size;
//        memcpy(data.data()+node*chunk_size, data_subview.data()+i*chunk_size, writesize);
//        node_list(node) = NodeID(node, cur_id+1);
//      } else {
//        node_list(node) = NodeID(node, cur_id-1);
//      }
//    });
//    Kokkos::parallel_for("Update node list", Kokkos::RangePolicy<>(0, num_chunks), KOKKOS_LAMBDA(const uint32_t i) {
//      if((node_list(i).tree == cur_id)) {
//        node_list(i) = NodeID(i, cur_id-1);
//      }
//    });
...
restart_chkpt_global(std::vector<Kokkos::View<uint8_t*>::HostMirror>& incr_chkpts, 
  ...
  Kokkos::parallel_for("Restart Hashlist distinct", Kokkos::RangePolicy<>(0, num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
    ...
    uint64_t src_offset = static_cast<uint64_t>(i) * static_cast<uint64_t>(chunk_size);
    uint64_t dst_offset = static_cast<uint64_t>(node) * static_cast<uint64_t>(chunk_size);
    first_ocur_map.insert(NodeID(node,cur_id),  src_offset);
    node_list(node) = NodeID(node, cur_id);
    uint32_t datasize = chunk_size;
    if(node == num_chunks-1)
      datasize = datalen - dst_offset;
    memcpy(data.data()+dst_offset, data_subview.data()+src_offset, datasize);
  });
  ...
  Kokkos::parallel_for("Load duplicate counts", Kokkos::RangePolicy<>(0,num_prior_chkpts), KOKKOS_LAMBDA(const uint32_t i) {
    uint32_t chkpt;
    uint64_t chkpt_offset = static_cast<uint64_t>(i)*2*sizeof(uint32_t);
    memcpy(&chkpt, dupl_count_subview.data()+chkpt_offset, sizeof(uint32_t));
    memcpy(&repeat_region_sizes(chkpt), dupl_count_subview.data()+chkpt_offset+sizeof(uint32_t), sizeof(uint32_t));
...
//    uint32_t idx = first_ocur_map.find(NodeID(prev, tree));
    ...
    if(tree == cur_id) {
      ...
      uint64_t dst_offset = static_cast<uint64_t>(node) * static_cast<uint64_t>(chunk_size);
      if(node == num_chunks-1)
        copysize = data.size() - dst_offset;
      memcpy(data.data()+dst_offset, data_subview.data()+src_offset, copysize);
    }
  });
  ...
  for(int idx=static_cast<int>(chkpt_idx)-1; idx>=static_cast<int>(ref_id) && idx < chkpt_idx; idx--) {
    ...
    Kokkos::parallel_for("Fill distinct map", Kokkos::RangePolicy<>(0, num_first_ocur), KOKKOS_LAMBDA(const uint64_t i) {
      uint32_t node;
      memcpy(&node, first_ocur_subview.data()+i*sizeof(uint32_t), sizeof(uint32_t));
      first_ocur_map.insert(NodeID(node,cur_id), i*static_cast<uint64_t>(chunk_size));
    });
    Kokkos::View<uint32_t*> repeat_region_sizes("Repeat entires per chkpt", cur_id+1);
    Kokkos::parallel_for("Load repeat map", Kokkos::RangePolicy<>(0,num_prior_chkpts), KOKKOS_LAMBDA(const uint64_t i) {
      uint32_t chkpt;
...
//      memcpy(&repeat_region_sizes(chkpt), chkpt_buffer_d.data()+dupl_count_offset+i*2*sizeof(uint32_t)+sizeof(uint32_t), sizeof(uint32_t));
    ...
    Kokkos::parallel_for("Restart Hash tree repeats middle chkpts", Kokkos::RangePolicy<>(0,num_repeat_entries), KOKKOS_LAMBDA(const uint64_t i) { 
    ...
    Kokkos::parallel_for("Fill data middle chkpts", Kokkos::RangePolicy<>(0, num_chunks), KOKKOS_LAMBDA(const uint32_t i) {
      if(node_list(i).tree == current_id) {
        ...
        if(first_ocur_map.exists(id)) {
          size_t src_offset = first_ocur_map.value_at(first_ocur_map.find(id));
          ...
          memcpy(data.data()+dst_offset, data_subview.data()+src_offset, writesize);
          ...
          if(prev.tree == current_id) {
            ...
            size_t src_offset = first_ocur_map.value_at(first_ocur_map.find(prev));
            size_t dst_offset = static_cast<size_t>(i)*static_cast<size_t>(chunk_size);
            uint32_t writesize = chunk_size;
            if(dst_offset+writesize > datalen) 
              writesize = datalen-dst_offset;
...
//    node_list(node) = NodeID(prev, tree);
////printf("Added entry %u: (%u,%u)\n", node, prev, tree);
//    if(tree == cur_id) {
//      uint32_t copysize = chunk_size;
//      if(node == num_chunks-1)
//        copysize = data.size() - chunk_size*node;
//      memcpy(data.data()+chunk_size*node, data_subview.data()+offset, copysize);
//printf("Copied data for %u\n", i);
//    }
//  });
...
//if((team_member.team_rank() == 0)) // && (cur_id > 0) && ((src_offset+data_offset > chkpt_size) || (dst_offset+data_offset > chkpt_size)))
//printf("Src offset: %lu, Dst offset: %lu\n", src_offset, dst_offset);
      if(node == num_chunks-1)
        datasize = datalen - dst_offset;
...
write_incr_chkpt_hashlist_basic( const DataView& data, 
  ...
  buffer_size += static_cast<uint64_t>(changes.size())*(sizeof(uint32_t) + static_cast<uint64_t>(chunk_size));
...
////  Kokkos::deep_copy(changes.vector_h, changes.vector_d);
  ...
  size_t data_offset = static_cast<uint64_t>(changes.size())*sizeof(uint32_t);
  Kokkos::View<uint8_t*, Kokkos::MemoryTraits<Kokkos::Unmanaged> > changes_bytes((uint8_t*)(changes.data()), data_offset);
  auto data_subview = Kokkos::subview(buffer_d, std::make_pair(sizeof(header_t), sizeof(header_t)+data_offset));
  Kokkos::deep_copy(data_subview, changes_bytes);
...
//  bitset.reset();
//
//  printf("Hashlist Bitset count: %u\n", bitset.count());
//  uint32_t num_changes = changes.size();
...
//    bitset.set(chunk);
//    uint64_t offset = static_cast<uint64_t>(num_changes)*sizeof(uint32_t);
//    uint32_t writesize = chunk_size;
//    if(chunk == num_chunks-1) {
//      writesize = data.size()-chunk*chunk_size;
//    }
//if(i < 100) {
//    printf("Writing entry %u data %lu to buffer offset %lu (i:%u, offset:%lu, num_changes:%u)\n", i, static_cast<uint64_t>(chunk_size*chunk), offset+i*chunk_size, i, offset, num_changes);
//}
//    memcpy(buffer_d.data()+sizeof(header_t)+i*sizeof(uint32_t), &chunk, sizeof(uint32_t));
//    memcpy(buffer_d.data()+sizeof(header_t)+offset+i*chunk_size, data.data()+chunk_size*chunk, writesize);
//  });
//  printf("Hashlist Bitset count: %u\n", bitset.count());
...
//      writesize = data.size()-chunk*chunk_size;
//    }
//    uint64_t dst_offset = static_cast<uint64_t>(i)*static_cast<uint64_t>(chunk_size);
//    uint64_t src_offset = static_cast<uint64_t>(chunk)*static_cast<uint64_t>(chunk_size);
//    memcpy(buffer_d.data()+sizeof(header_t)+data_offset+dst_offset, data.data()+src_offset, writesize);
//  });

  uint64_t num_changes = static_cast<uint64_t>(changes.size());
  Kokkos::parallel_for("Copy data", Kokkos::TeamPolicy<>(num_changes, Kokkos::AUTO()), 
                         KOKKOS_LAMBDA(const Kokkos::TeamPolicy<>::member_type& team_member) {
    uint32_t i = team_member.league_rank();
    uint32_t chunk = changes(i);
    uint32_t writesize = chunk_size;
    uint64_t src_offset = static_cast<uint64_t>(chunk_size)*static_cast<uint64_t>(chunk);
    uint64_t dst_offset = num_changes*sizeof(uint32_t)+static_cast<uint64_t>(i)*static_cast<uint64_t>(chunk_size);
    if(chunk == num_chunks-1) {
      writesize = data.size()-src_offset;
    }
    uint32_t* buffer_u32 = (uint32_t*)(buffer_d.data()+sizeof(header_t)+dst_offset);
    uint32_t* data_u32 = (uint32_t*)(data.data()+src_offset);
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, writesize/4), [&] (const uint64_t& j) {
      buffer_u32[j] = data_u32[j];
    });
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, writesize%4), [&] (const uint64_t& j) {
      buffer_d(sizeof(header_t)+dst_offset+((writesize/4)*4)+j) = data(src_offset+((writesize/4)*4)+j);
    });
  });
  ...
  STDOUT_PRINT("Num shift dupl: %u\n"  , header.num_shift_dupl);
  STDOUT_PRINT("Num prior chkpts: %u\n", header.num_prior_chkpts);
  uint64_t num_data     = static_cast<uint64_t>(header.num_first_ocur)*static_cast<uint64_t>(chunk_size);
...
//      }
//      memcpy(buffer_d.data()+sizeof(header_t) + data_offset+(pos/sizeof(uint32_t))*static_cast<uint64_t>(chunk_size), data.data()+static_cast<uint64_t>(chunk_size)*static_cast<uint64_t>(i), writesize);
//    }
//  });
  Kokkos::parallel_for("Make incremental checkpoint", Kokkos::TeamPolicy<>(changes.size(), Kokkos::AUTO()), 
                         KOKKOS_LAMBDA(const Kokkos::TeamPolicy<>::member_type& team_member) {
    uint32_t i = team_member.league_rank();
    if(changes.test(i)) {
      size_t pos = 0;
...
//STDOUT_PRINT("Writing %u:%u to %lu\n", i, num_repeats_i, pos);
...
//STDOUT_PRINT("Writing %u:%u to %lu\n", node, prev.node, shift_dupl_offset+dupl_offset);
...
//STDOUT_PRINT("Copying data\n");
  Kokkos::parallel_for("Copy data", Kokkos::TeamPolicy<>(first_ocur.size(), Kokkos::AUTO()), 
    ...
    uint32_t* data_u32 = (uint32_t*)(data.data()+src_offset);
//STDOUT_PRINT("Writing %u bytes for %u from %lu to %lu\n", writesize, chunk, src_offset, dst_offset);
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, writesize/4), [&] (const uint64_t& j) {
      buffer_u32[j] = data_u32[j];
    });
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, writesize%4), [&] (const uint64_t& j) {
