...
//restart_chkpt_local(std::vector<Kokkos::View<uint8_t*>::HostMirror>& incr_chkpts, 
//                             const int chkpt_idx, 
...
//    Kokkos::parallel_for("Calculate num chunks", Kokkos::RangePolicy<>(0, num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
//      uint32_t node;
...
//    });
//
//    Kokkos::parallel_scan("Calc offsets", num_first_ocur, KOKKOS_LAMBDA(const uint32_t i, uint32_t& partial_sum, bool is_final) {
//      const uint32_t len = chunk_len(i);
//      if(is_final) chunk_len(i) = partial_sum;
//      partial_sum += len;
//    });
//
//    Kokkos::parallel_for("Restart Hashtree distinct", Kokkos::RangePolicy<>(0,num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
...
//    });
//
//    Kokkos::parallel_for("Restart Hashtree current repeat", Kokkos::RangePolicy<>(0, num_shift_dupl), KOKKOS_LAMBDA(const uint32_t i) {
//      uint32_t node;
//      uint32_t prev;
...
//std::pair<double,double>
//restart_chkpt_local(std::vector<std::vector<uint8_t>>& incr_chkpts, 
//                             const int chkpt_idx, 
//                             Kokkos::View<uint8_t*>& data,
//                             size_t size,
//                             uint32_t num_chunks,
...
////    auto chkpt_buffer_h = Kokkos::create_mirror_view(chkpt_buffer_d);
////    memcpy(chkpt_buffer_h.data(), incr_chkpts[header.ref_id].data(), chkpt_size);
//    chkpt_buffer_h = incr_chkpts[header.ref_id];
//    t2 = std::chrono::high_resolution_clock::now();
...
//    num_first_ocur = chkpt_header.distinct_size;
//    Kokkos::resize(distinct_nodes, num_first_ocur);
//    Kokkos::resize(chunk_len, num_first_ocur);
//Kokkos::Profiling::popRegion();
//Kokkos::Profiling::pushRegion("Checkpoint "+std::to_string(header.ref_id)+" Load maps");
//    Kokkos::parallel_for("Tree:Reference:Calculate num chunks", Kokkos::RangePolicy<>(0, num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
//      uint32_t node;
//      memcpy(&node, distinct.data()+i*sizeof(uint32_t), sizeof(uint32_t));
//      uint32_t len = num_leaf_descendents(node, num_nodes);
//      distinct_nodes(i) = node;
//      chunk_len(i) = len;
//    });
//    Kokkos::parallel_scan("Tree:Reference:Calc offsets", num_first_ocur, KOKKOS_LAMBDA(const uint32_t i, uint32_t& partial_sum, bool is_final) {
//      const uint32_t len = chunk_len(i);
//      if(is_final) chunk_len(i) = partial_sum;
//      partial_sum += len;
//    });
//
//    Kokkos::parallel_for("Tree:Reference:Fill distinct map", Kokkos::RangePolicy<>(0,num_first_ocur), KOKKOS_LAMBDA(const uint32_t i) {
//      uint32_t node = distinct_nodes(i);
//      distinct_map.insert(NodeID(node, cur_id), chunk_len(i)*chunk_size);
////      DEBUG_PRINT("Reading region: (%u,%u) at %lu: %lu\n", node, cur_id, i*sizeof(uint32_t), data_offset+chunk_len(i)*chunk_size);
//    });
...
////      }
////    });
//    Kokkos::parallel_for("Tree:Reference:Fill data reference chkpt", Kokkos::TeamPolicy<>(num_chunks, Kokkos::AUTO()), KOKKOS_LAMBDA(const Kokkos::TeamPolicy<>::member_type& team_member) {
//      uint32_t i = team_member.league_rank();
//      if(node_list(i).tree == current_id) {
//        NodeID id = node_list(i);
//        uint32_t first_ocur_idx = distinct_map.find(id);
//        if(distinct_map.valid_at(first_ocur_idx)) {
//          uint32_t len = num_leaf_descendents(id.node, num_nodes);
//          if(team_member.team_rank() == 0)
//            Kokkos::atomic_add(&total_region_size(0), len);
//          uint32_t start = leftmost_leaf(id.node, num_nodes);
//          size_t offset = distinct_map.value_at(first_ocur_idx);
//          uint32_t writesize = chunk_size*len;
//          if(team_member.team_rank() == 0)
//            Kokkos::atomic_add(&counter_d(0), writesize);
//          if((start+len-1-num_chunks+1)*chunk_size >= datalen)
...
//std::pair<double,double> 
//restart_chkpt_global(std::vector<std::vector<uint8_t> >& incr_chkpts, 
//                             const int chkpt_idx, 
//                             Kokkos::View<uint8_t*>& data,
//                             size_t size,
//                             uint32_t num_chunks,
...
//    std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
//    Kokkos::resize(buffer_d, size);
//    Kokkos::resize(buffer_h, size);
//    memcpy(buffer_h.data(), incr_chkpts[chkpt_idx].data(), incr_chkpts[chkpt_idx].size());
//    std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
//    STDOUT_PRINT("Time spent reading checkpoint %u from file: %f\n", chkpt_idx, (1e-9)*(std::chrono::duration_cast<std::chrono::nanoseconds>(t2-t1).count()));
//    Kokkos::fence();
//Kokkos::Profiling::popRegion();
...
//std::pair<double,double> 
//restart_incr_chkpt_hashtree( std::vector<std::vector<uint8_t>>& incr_chkpts,
//                             const int idx, 
//                             Kokkos::View<uint8_t*>& data) {
//  // Read main incremental checkpoint header
//  size_t filesize = incr_chkpts[idx].size();
...
//  // Only deduplicated with baseline checkpoint
//  if(header.window_size == 0) {
//    times = restart_chkpt_local(incr_chkpts, idx, data, filesize, num_chunks, num_nodes, header, buffer_d, buffer_h);
//  } else { // Checkpoint uses full history of checkpoints for deduplication
//    times = restart_chkpt_global(incr_chkpts, idx, data, filesize, num_chunks, num_nodes, header, buffer_d, buffer_h);
//  }
//  Kokkos::fence();
//  DEBUG_PRINT("Restarted checkpoint\n");
//  return times;
