...
//  Kokkos::parallel_for("Baseline: Leaves", Kokkos::RangePolicy<>(num_chunks-1, num_nodes), KOKKOS_LAMBDA(const uint32_t leaf) {
...
//  });
  ...
  Kokkos::TeamPolicy<> team_policy = Kokkos::TeamPolicy<>(((num_nodes-num_chunks+1)/32)+1, 32);
...
//          tree_roots.push(0);
//          first_ocur_updates.insert(node, NodeID(node, chkpt_id));
      }
    });
...
//            NodeID node_l = first_occur_d.value_at(first_occur_d.find(curr_tree(child_l)));
            if(labels(child_l) == SHIFT_DUPL) {
//              shift_dupl_updates.insert(child_l, node_l);
              shift_dupl_updates.push(child_l);
            } else {
//              first_ocur_updates.insert(node_l.node, node_l);
...
//            NodeID node_r = first_occur_d.value_at(first_occur_d.find(curr_tree(child_r)));
            if(labels(child_r) == SHIFT_DUPL) {
//              shift_dupl_updates.insert(child_r, node_r);
              shift_dupl_updates.push(child_r);
            } else {
//              first_ocur_updates.insert(node_r.node, node_r);
...
//  // Count regions
//  Kokkos::parallel_for("Baseline: Count regions", Kokkos::RangePolicy<>(0, tree_roots.size()), KOKKOS_LAMBDA(const uint32_t i) {
////    auto chunk_counters_sa = chunk_counters_sv.access();
////    auto region_counters_sa = region_counters_sv.access();
...
//  Kokkos::parallel_for(leaves_label, Kokkos::RangePolicy<>(num_chunks-1, num_nodes), KOKKOS_LAMBDA(const uint32_t leaf) {
//    auto chunk_counters_sa = chunk_counters_sv.access();
//    auto region_counters_sa = region_counters_sv.access();
//    uint32_t num_bytes = chunk_size;
//    uint64_t offset = static_cast<uint64_t>(leaf-(num_chunks-1))*static_cast<uint64_t>(chunk_size);
...
//    Kokkos::TeamPolicy<> team_policy = Kokkos::TeamPolicy<>(((level_end+1-level_beg)/32)+1, 32);
//    Kokkos::parallel_for(first_ocur_forest_label, team_policy, 
//    KOKKOS_LAMBDA(member_type team_member) {
//      uint64_t i=team_member.league_rank();
//      uint64_t j=team_member.team_rank();
//      uint64_t node = level_beg+i*team_member.team_size()+j;
      if(node < num_chunks-1) {
        uint32_t child_l = 2*node+1;
        uint32_t child_r = 2*node+2;
        if(labels(child_l) == FIRST_OCUR && labels(child_r) == FIRST_OCUR) {
...
//    Kokkos::TeamPolicy<> team_policy = Kokkos::TeamPolicy<>(((level_end+1-level_beg)/32)+1, 32);
//    Kokkos::parallel_for(forest_label, team_policy, 
//    KOKKOS_LAMBDA(member_type team_member) {
//      uint64_t i=team_member.league_rank();
//      uint64_t j=team_member.team_rank();
//      uint64_t node = level_beg+i*team_member.team_size()+j;
...
//  // Count regions
//  std::string count_label = std::string("Checkpoint ") + std::to_string(chkpt_id) + std::string(": Count Regions");
//  Kokkos::parallel_for(count_label, Kokkos::RangePolicy<>(0,tree_roots.size()), KOKKOS_LAMBDA(const uint32_t i) {
////    auto chunk_counters_sa = chunk_counters_sv.access();
////    auto region_counters_sa = region_counters_sv.access();
//    uint32_t root = tree_roots.vector_d(i);
//    if(labels(root) != DONE) {
